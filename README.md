...
to the original pose. The grasp path uses denser contact, wrap, hold, and
release anchors so the fingers can stay on the can surface during the close.
...
- `web/public/demo/hand_sequence/prediction.csv`: generated prediction output
  ...
  web test path has 64 grasp-can keyframes, so it produces

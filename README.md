...
- `Prediction` mode uses the complete `sample_keyframes.csv` list for keyframe
  selection and builds its own sample-derived prediction segments for the table.
  It does not display runtime IO segment ids. It can show segment type,
...
| Prediction | enabled | sample_keyframes.csv selection plus sample-derived prediction frames | runtime_io ids in the prediction table |

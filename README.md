...
By default the writer loop uses `RANDOM_INIT=1`, `KEYFRAME_MODE=gesture`,
`KEYFRAME_COUNT=12`, `NOISE_SCALE=1.0`, and `FRAMES=1000`. Every loop
generates a new noisy gesture-like keyframe path, writes 1000 predicted rows
between each pair of yellow keyframe anchors, and appends completion metrics to
`result.json`.
...
- `web/public/demo/hand_sequence/prediction.csv`: generated prediction output
  ...
  web test path has 12 gesture keyframes, so it produces
  `12 + (12 - 1) * 1000 = 11012` rows.
...
```sh
WEB_SEED=222 NOISE_SCALE=1.5 KEYFRAME_COUNT=12 FRAMES=1000 make -f Makefile.linux web
```

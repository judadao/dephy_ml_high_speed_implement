...
Positive, negative, and test dataset design is recorded in
...
- `Prediction` mode uses the complete `sample_keyframes.csv` list for keyframe
  selection and shows sample-derived prediction rows under those sample
  keyframes. It does not display runtime IO segment ids and does not show a
  second sample/reference panel.
- `Anchors` mode plays and inspects the complete `sample_keyframes.csv` script.
  It is sample-keyframe-only and does not show prediction rows.
...
- `web/src/playbackController.js`: tab playback state rules and segment frame stepping.
- `web/src/manualPlayback.js`: manual anchor and prediction selection helpers.
- `web/src/playbackTiming.js`: segment duration timing.
...
| Mode | Play button | Hand motion source | Must not happen |
| --- | --- | --- | --- |
| Realtime Demo | enabled | current SSE prediction segment frames | running with no frame advance |
| Prediction | enabled | sample_keyframes.csv keyframes plus sample-derived prediction frames | runtime_io ids or a duplicate sample panel in the prediction table |
| Anchors | enabled | complete sample_keyframes.csv frames only | prediction rows or latest-15 runtime anchor truncation |

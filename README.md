...
- `web/src/AnchorsTab.jsx`: raw runtime anchor manual review UI.
...
`Anchors` is manual review only; selecting a row updates the hand, and the
global Play button is disabled in this mode.
...
| Prediction | enabled | selected/generated prediction segment frames | reset to segment start when resuming |
| Anchors | disabled | manual row selection from an entry snapshot | automatic anchor replay or live row scrolling |

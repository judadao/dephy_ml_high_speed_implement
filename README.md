...
- `Keyframes` mode shows a compact keyframe list only. Selecting a keyframe
  pauses playback and hides prediction rows so recorded script review stays
  simple.
- `Prediction` mode highlights the currently active keyframe interval. The
  prediction rows are folded under their source keyframe and can be expanded
  only when the intermediate CSV values need inspection.
- The active keyframe row is kept centered inside the small keyframe panel

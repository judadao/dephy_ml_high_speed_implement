...
- `Prediction` mode uses the complete `sample_keyframes.csv` list for keyframe
  selection while inspecting generated prediction segments and segment rows.
  It can show segment type, from-anchor, to-anchor, target kind, row progress,
  and confidence.
- `Anchors` mode plays and inspects the complete `sample_keyframes.csv` script.
  It does not use the latest-15 runtime IO anchor display limit.
...
| Prediction | enabled | sample_keyframes.csv selection plus generated prediction segment frames | reset to segment start when resuming |
| Anchors | enabled | complete sample_keyframes.csv frames | latest-15 runtime anchor truncation |

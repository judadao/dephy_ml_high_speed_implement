...
`KEYFRAME_COUNT=40`, `NOISE_SCALE=1.0`, and `FRAMES=1000`. Every loop
...
metrics to `result.json`. The default web scene shows a fixed can-shaped column:
the hand starts open, reaches the can, closes around it, releases, and returns
...
KEYFRAME_COUNT=40 KEYFRAME_MODE=grasp_can make -f Makefile.linux web-sequence-demo-loop
...
- `web/public/demo/hand_sequence/prediction.csv`: generated prediction output
  ...
  web test path has 40 grasp-can keyframes, so it produces
  `40 + (40 - 1) * 1000 = 39040` rows.
...
```sh
WEB_SEED=222 NOISE_SCALE=1.5 KEYFRAME_COUNT=40 FRAMES=1000 make -f Makefile.linux web
```
